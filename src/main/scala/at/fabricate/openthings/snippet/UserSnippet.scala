...
//              bindFieldsCSS(xhtml)
  ...
  def queryItems[T <: BaseEntityWithTitleAndDescription[T]](itemToQuery: BaseEntityWithTitleAndDescription[T], limit : Boolean ) = { // , otherQueryParams = List(StartAt(curPage*itemsPerPage), MaxRows(itemsPerPage))
  ...
  override def count = queryItems[User](User,false).length
  
  override def page = queryItems[User](User,true)
}
